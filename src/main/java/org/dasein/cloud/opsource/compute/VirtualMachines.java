...
public class VirtualMachines implements VirtualMachineSupport {
    ...
    public boolean attachDisk(String serverId, int sizeInGb) throws InternalException, CloudException {
        ...
        Param param = new Param(OpSource.SERVER_BASE_PATH, null);
     	...
     	return method.parseRequestResult("Attaching disk", doc , "ns9:result","ns9:resultDetail");
    }
    ...
    public VirtualMachine getVirtualMachineByName(String name) throws InternalException, CloudException {
    	...
    	for(VirtualMachine vm : list ){
     	...
     	list = (ArrayList<VirtualMachine>) listDeployServers();
    	for(VirtualMachine vm : list ){
    		if(vm.getName().equals(name)){
    			return vm;  			      			
    		}    		
    ...
    public Iterable<VirtualMachineProduct> listProducts(Architecture architecture) throws InternalException, CloudException {
		...
		if(blocks != null){
			for(int i=0; i< blocks.getLength();i++){
				...
				RegionComputingPower r = toRegionComputingPower(item);
    ...
    private Iterable<VirtualMachine> listDeployServers() throws InternalException, CloudException {
    	...
    	Document doc = method.invoke();
    	System.out.println("doc -> " + provider.convertDomToString(doc));
    	NodeList matches = doc.getElementsByTagName(Deployed_Server_Tag);
   ...
   private Iterable<VirtualMachine> listePendingServers() throws InternalException, CloudException {
    ...
    public void pause(String serverId) throws InternalException, CloudException {
     	...
     	OpSourceMethod method = new OpSourceMethod(provider, 
     			...
     			provider.getBasicRequestParameters(OpSource.Content_Type_Value_Single_Para, "GET",null));
       	method.parseRequestResult("Pauseing vm",method.invoke(),"ns9:result","ns9:resultDetail");      
    }
    ...
    public void reboot(String serverId) throws CloudException, InternalException {
    	...
    	OpSourceMethod method = new OpSourceMethod(provider, 
    			...
    			provider.getBasicRequestParameters(OpSource.Content_Type_Value_Single_Para, "GET",null));
    	method.parseRequestResult("Rebooting vm",method.invoke(),"ns9:result","ns9:resultDetail");
    }
    			...
    			try {    				
    				...
    				if(server.getCurrentState().equals(VmState.RUNNING)){
        				...
        				if(resultCode.equals("REASON_0")){
        					...
        					logger.error("The server " + serverId + " is associated with a Real-Server in load balancer");
        					localAttemptToTerminateVM = 0;  
  ...
  private String killVM(String serverId) throws InternalException, CloudException {
    ...
    private VirtualMachine toVirtualMachine(Node node, Boolean isPending) throws CloudException, InternalException {
        if( node == null ) {
            return null;
        }
        ...
        for( int i=0; i<attributes.getLength(); i++ ) {
            ...
            else if( name.equalsIgnoreCase("vlanResourcePath") ) {
            ...
            //DeployedServer
           else if( name.equalsIgnoreCase("publicIpAddress") ) { 
               server.setPublicIpAddresses(new String[] { value });               
           }
           else if( name.equalsIgnoreCase("isDeployed") ) {
           ...
           else if( name.equalsIgnoreCase("isStarted") ) {
            ...
            //From here is the deployed server, or pending server
           else if( name.equalsIgnoreCase("machineSpecification") ) {
           		...
           		for(int j=0;j<machineAttributes.getLength();j++ ){
	 	            ...
	 	            if( machine.getNodeName().equalsIgnoreCase("operatingSystem") ){
...
//	 		                }
	 	            ...
	 	            }else if( machine.getNodeName().equalsIgnoreCase("cpuCount") && machine.getFirstChild().getNodeValue() != null ) {
	                 	server.getTags().put("cpuCount", machine.getFirstChild().getNodeValue());
	                }
	 	            //memoryMb pendingDeploy deployed
	                else if( (machine.getNodeName().equalsIgnoreCase("memory") || machine.getNodeName().equalsIgnoreCase("memoryMb"))&& machine.getFirstChild().getNodeValue() != null ) {
	                 	server.getTags().put("memory", machine.getFirstChild().getNodeValue());
	                }
	 	            //deployedserver osStorageGb
	                else if( (machine.getNodeName().equalsIgnoreCase("osStorage") ||machine.getNodeName().equalsIgnoreCase("osStorageGb"))&& machine.getFirstChild().getNodeValue() != null) { 
	              	   server.getTags().put("osStorage", machine.getFirstChild().getNodeValue());
	                }
	 	            //additionalLocalStorageGb pendingDeploy
	                else if( (machine.getNodeName().equalsIgnoreCase("additionalLocalStorage") || machine.getNodeName().equalsIgnoreCase("additionalLocalStorageGb") ) && machine.getFirstChild().getNodeValue() != null ) { 
	              	   server.getTags().put("additionalLocalStorage", machine.getFirstChild().getNodeValue());
	                }                     
           		}           
       	   }
            // pendingDeploy or Deployed
           else if( name.equalsIgnoreCase("sourceImageId") ) {
        	   server.setProviderMachineImageId(value);
           }
            // pendingDeploy or Deployed
           else if( name.equalsIgnoreCase("networkId") ) {
          ...
          //From here is the specification for pending deployed server
	            ...
	            if( status.getNodeName().equalsIgnoreCase("step") ){
	            	//TODO
	         /*   	NodeList stepAttributes  = status.getChildNodes();
		            for(int k=0;k<stepAttributes.getLength();k++ ){
		            	Node step = stepAttributes.item(j);
		            	String stepName = step.getNodeName().toLowerCase();              
		                String stepValue = null ;
		                if( stepName.equals("name") && step.getChildNodes().getLength() > 0 ) {
		                	stepValue = step.getFirstChild().getNodeValue();
		                }else if( stepName.equals("number") && step.getChildNodes().getLength() > 0) {
		                	stepValue = step.getFirstChild().getNodeValue();
		                				                       
		                }        
		            	          		     		 
		            }*/
	            }else if( status.getNodeName().equalsIgnoreCase("requestTime") && status.getFirstChild().getNodeValue() != null ) {
	            ...
	            else if( status.getNodeName().equalsIgnoreCase("userName") && status.getFirstChild().getNodeValue() != null ) {
                	server.setProviderOwnerId(status.getFirstChild().getNodeValue());
	            }
	            else if( status.getNodeName().equalsIgnoreCase("numberOfSteps") ) { 
	             	
	            }
	            else if( status.getNodeName().equalsIgnoreCase("action") ) { 
